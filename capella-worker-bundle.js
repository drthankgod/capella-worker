/**
 * ============================================================
 *  CAPELLA — Cloudflare Worker (single-file bundle)
 * ============================================================
 *  This is every module (auth, withdrawal, finance, activation,
 *  task, topup, otp, music, storage, cleanup, worker) combined
 *  into one file so it can be pasted straight into the Cloudflare
 *  dashboard's "Edit code" editor — no wrangler CLI needed.
 *
 *  Bindings required (set in dashboard Settings > Bindings):
 *    - D1 database binding named "DB" -> capella-db
 *
 *  Secrets required (set in dashboard Settings > Variables/Secrets,
 *  as "Secret" type, not plain text):
 *    - PAYSTACK_SECRET_KEY
 *    - AUTH_SECRET  (long random string, e.g. from a password generator)
 *
 *  Cron Trigger required (dashboard Settings > Triggers > Cron Triggers):
 *    - 0 * * * *   (every hour, runs the screenshot cleanup)
 * ============================================================
 */
 ...
 *   - Token lifetime: 30 days. There's no refresh-token flow yet —
 *     re-login once it expires. Good enough to start; can be split into
 *     access/refresh tokens later if you want shorter-lived sessions.
 *
...
const auth = { register, login, requireAuth, signToken, verifyToken, hashPassword, verifyPassword };
...
const withdrawal = { listBanks, resolveBankAccount, requestWithdrawal, requestReserveWithdrawal, transferWebhook };
...
const finance = { createCampaign, cancelCampaign, reviewSubmission };
...
const activation = { verifyActivationPayment };
...
const topup = { topUpBusinessWallet };
...
const otp = { listPendingOtpTransfers, finalizeTransferOtp };
 ...
 * ---------------------------------------------------------------------
 * Ported from functions/music.js — purchase logic only. File upload and
 * serving now live in storage.js (D1 BLOBs, since R2 needs a card on
 * file to enable). See storage.js for uploadSongFile / streamSongFile.
 *
 * Routes (mounted in worker.js):
...
async function purchaseSong(request, env) {
  ...
  if (song.status !== "active") return json({ success: false, message: "This song isn't available for purchase right now." });
...
const storage = { uploadTaskProof, uploadSongFile, streamSongFile, MAX_PROOF_BYTES, MAX_SONG_BYTES };
...
/**
 * Capella — Task-Proof Retention (Cloudflare Workers Cron Trigger)
 * -----------------------------------------------------------------
 * Ported from functions/cleanup.js. Firebase's onSchedule('every 1
 * hours') becomes a Cron Trigger, configured in wrangler.toml:
 *
 *   [triggers]
 ...
 *
 * Screenshots live as BLOBs in D1 (see storage.js) rather than R2 — R2
 * needs a card on file to enable at all, so this deletes expired rows
 * from task_proof_files directly. Submission rows themselves are NOT
 * touched, same as the original: only the image bytes are deleted,
...
const cleanup = { deleteExpiredTaskProofs };
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (pathname === "/auth/register" && request.method === "POST") {
        return withCORS(await auth.register(request, env));
      }
      if (pathname === "/auth/login" && request.method === "POST") {
        return withCORS(await auth.login(request, env));
      }

      // ---- withdrawals ----
      if (pathname === "/banks" && request.method === "GET") {
        return withCORS(await withdrawal.listBanks(request, env));
      }
      if (pathname === "/banks/resolve" && request.method === "POST") {
        return withCORS(await withdrawal.resolveBankAccount(request, env));
      }
      if (pathname === "/withdrawals" && request.method === "POST") {
        return withCORS(await withdrawal.requestWithdrawal(request, env));
      }
      if (pathname === "/admin/reserve-withdrawals" && request.method === "POST") {
        return withCORS(await withdrawal.requestReserveWithdrawal(request, env));
      }
      ...
      if (pathname === "/campaigns" && request.method === "POST") {
        return withCORS(await finance.createCampaign(request, env));
      }
      const cancelMatch = pathname.match(/^\/campaigns\/([^/]+)\/cancel$/);
      if (cancelMatch && request.method === "POST") {
        return withCORS(await finance.cancelCampaign(request, env, cancelMatch[1]));
      }
      const approveMatch = pathname.match(/^\/admin\/submissions\/([^/]+)\/approve$/);
      if (approveMatch && request.method === "POST") {
        return withCORS(await finance.reviewSubmission(request, env, approveMatch[1], true));
      }
      const rejectMatch = pathname.match(/^\/admin\/submissions\/([^/]+)\/reject$/);
      if (rejectMatch && request.method === "POST") {
        return withCORS(await finance.reviewSubmission(request, env, rejectMatch[1], false));
      }

      // ---- activation ----
      if (pathname === "/activations/verify" && request.method === "POST") {
        return withCORS(await activation.verifyActivationPayment(request, env));
      }

      // ---- task submission + auto-verification ----
      if (pathname === "/submissions" && request.method === "POST") {
        return withCORS(await task.createSubmission(request, env));
      }

      // ---- wallet top-up ----
      if (pathname === "/wallet/topup" && request.method === "POST") {
        return withCORS(await topup.topUpBusinessWallet(request, env));
      }

      // ---- OTP transfer finalization (admin) ----
      if (pathname === "/admin/otp-transfers" && request.method === "GET") {
        return withCORS(await otp.listPendingOtpTransfers(request, env));
      }
      if (pathname === "/admin/otp-transfers/finalize" && request.method === "POST") {
        return withCORS(await otp.finalizeTransferOtp(request, env));
      }

      // ---- music marketplace ----
      if (pathname === "/music/purchase" && request.method === "POST") {
        return withCORS(await music.purchaseSong(request, env));
      }

      // ---- file storage (D1 BLOBs — see storage.js) ----
      if (pathname === "/task-proofs" && request.method === "POST") {
        return withCORS(await storage.uploadTaskProof(request, env));
      }
      const songUploadMatch = pathname.match(/^\/musicians\/songs\/([^/]+)\/file$/);
      if (songUploadMatch && request.method === "POST") {
        return withCORS(await storage.uploadSongFile(request, env, songUploadMatch[1]));
      }
      ...
      if (songStreamMatch && request.method === "GET") {
        ...
        return withCORS(await storage.streamSongFile(request, env, songStreamMatch[1]));
      }
