/**
 * ============================================================
 *  CAPELLA — Cloudflare Worker (single-file bundle, v2)
 * ============================================================
 *  Every module wrapped in its own IIFE so internal helper names
 *  (json, n, isAdmin, etc.) can't collide across modules — that
 *  collision was the cause of the first failed build.
 *
 *  Bindings required (dashboard Settings > Bindings):
 *    - D1 database binding named "DB" -> capella-db
 *
 *  Secrets required (dashboard Settings > Variables/Secrets, as
 *  "Secret" type):
 *    - PAYSTACK_SECRET_KEY
 *    - AUTH_SECRET
 *
 *  Cron Trigger required (dashboard Settings > Triggers):
 *    - 0 * * * *
 * ============================================================
 */
...
async function login(request, env) {
  ...
  return { register, login, requireAuth, signToken, verifyToken, hashPassword, verifyPassword };
})();
...
async function deleteExpiredTaskProofs(env) {
  ...
  return { deleteExpiredTaskProofs };
})();
